...
public class ServiceService {
    ...
    public Service createService(ServiceRequestDTO serviceDTO) {
        ...
        Location location = locationRepository.findById(serviceDTO.getLocationId())
        ...
        if (serviceExists) {
            throw new IllegalArgumentException(
                "Service '" + serviceDTO.getServiceName() + "' already exists in this location"
            );
        }
    ...
    public Service createServiceWithImage(ServiceRequestDTO serviceDTO, String imageUrl) {
        Location location = locationRepository.findById(serviceDTO.getLocationId())
            .orElseThrow(() -> new IllegalArgumentException(
                "Location with ID " + serviceDTO.getLocationId() + " not found"
            ));
    ...
    public List<Service> getAllServicesWithLocations() {
        return serviceRepository.findAllWithLocation();
    }
    ...
    public Service updateService(Integer id, ServiceRequestDTO serviceDTO) {
        ...
        if (serviceDTO.getLocationId() != null && 
            ...
            Location newLocation = locationRepository.findById(serviceDTO.getLocationId())
                .orElseThrow(() -> new IllegalArgumentException(
                    "Location with ID " + serviceDTO.getLocationId() + " not found"
                ));
    ...
    public boolean serviceExistsInLocation(String serviceName, Integer locationId) {
        return serviceRepository.existsByServiceNameAndLocationId(serviceName, locationId);
    }
}